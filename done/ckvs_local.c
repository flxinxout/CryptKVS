...
static int do_get(const ckvs_entry_t* p_entry, const CKVS_t* ckvs, const ckvs_memrecord_t* mr) {
    ...
    //---------- READ CIPHER ----------
    unsigned char* cipher = calloc(p_entry->value_len, sizeof(cipher[0])); //+1 to ensure the last '\0' char
    ...
    err_code = crypt_cipher(mr, 0, (unsigned char*) cipher,  p_entry->value_len, &data, NULL);
    if(err_code != ERR_NONE) {
        free(cipher);
        M_EXIT(err_code);
    }
...
static int do_set(ckvs_entry_t* p_entry, CKVS_t* ckvs, const ckvs_memrecord_t* mr, const char* set_value) {
    ...
    M_REQUIRE_NON_NULL(ckvs);
    M_REQUIRE_NON_NULL(mr);
    M_REQUIRE_NON_NULL(set_value);

    //---------- ENCRYPT VALUE ----------
    unsigned char* cipher = NULL;
    size_t written_size = 0;
    int err_code = crypt_cipher(mr, 1, (const unsigned char*) set_value, strlen(set_value), &cipher, &written_size);
    M_REQUIRE_NO_ERROR(err_code);
