...
/**
 * @brief Generates stretched_key, auth_key and c1 and stores them into the memrecord. Doesn't modify the memrecord
 * if an error occurs.
 *
...
/**
 * @brief Generates master key from c1 stored in mr and the given c2. Doesn't modify the memrecord
 * if an error occurs.
 *
...
/**
 * @brief Encrypt or Decrypt an incoming buffer with a certain size and place it into an leaving buffer with a certain
 * size too.
 *
 * The content of data is allocated. DO NOT FORGET TO FREE IT.
 *
 * @param mr (const ckvs_memrecord_t*) a pointer to a ckvs_memrecord_t
 * @param do_encrypt (int) 0 if we want to decrypt and 1 if we want to encrypt
 * @param inbuf (const unsigned char*) the buffer with the message to crypt/decrypt
 * @param inbuflen (size_t) the size of the message
 * @param outbuf (unsigned char**) a pointer to the buffer that will contain the message crypted/decrypted
 * @param outbuflen (size_t*) a pointer to where to store the effectively written size. Can be NULL.
 * @return int, an error code
 */
int crypt_cipher(const ckvs_memrecord_t* mr,
                 int do_encrypt,
