...
// ==========================================================================================
static int compute_stretched_key(const char* key, const char* pwd, ckvs_memrecord_t* mr) {
    size_t stretched_keylen = strnlen(key, CKVS_MAXKEYLEN) + strlen(STRETCHED_KEY_SEP) + strlen(pwd) + 1;
...
int crypt_cipher(const ckvs_memrecord_t* mr,
    ...
    M_REQUIRE_NON_NULL(mr);
    M_REQUIRE_NON_NULL(inbuf);
    M_REQUIRE_NON_NULL(outbuf);

    unsigned char* res = calloc(inbuflen + 1 + EVP_MAX_BLOCK_LENGTH, sizeof(res[0]));
    if(res == NULL) {
